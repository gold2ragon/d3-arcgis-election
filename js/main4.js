require([
	...
	function ChangeRace() {
		OnLayer.when(function() {
			...
			var Content =
				"<div id='ElectionResultsPopup' class='TableDisplay'><h4 style='font-size:1.25em'>" +
				CurrentRaceName +
				'</h4><table><tbody>';
			...
			for (var step = 0; step < Fields.length; step++) {
				...
				if (CurrentRaceList.indexOf(FieldName) >= 0) {
					...
					Content +=
						"<tr><td class='Party'><div class='PartyColor' style='background-color:" +
						NewColor +
						"'></div></td><td class='Candidate'>" +
						Candidate +
						"</td><td class='Votes'>{" +
						FieldName +
						"}</td><td class='Percent'>{expression/" +
						FieldName +
						'}%</td></tr>';
					ValueInfos.push({
						value: FieldName,
						symbol: { type: 'simple-fill', color: NewColor },
						label: CandidateNameList[FieldName],
					});
			...
			if (FieldList.length > 1) {
				...
				var ArcadeExpression =
					...
					ListStatement +
					')[' +
					String(FieldList.length - 1) +
					'] == Sort(' +
					ListStatement +
					')[' +
					String(FieldList.length - 2) +
					"], 'Tie', Decode(" +
					'Max(' +
					ListStatement +
					'), ' +
			...
			for (var step = 0; step < FieldList.length; step++) {
				...
				arcadeExpressionInfos.push({
					name: FieldName,
					title: FieldName + '_PERCENT',
					expression: 'Round(' + '($feature.' + FieldName + '/(' + AllFeatures + '))*100, 1)',
				});
			}
			...
			arcadeExpressionInfos.push({
				name: 'ProperName',
				title: 'Proper Name',
				expression: "(Proper($feature.NAME, 'everyword'))",
			});
			...
			arcadeExpressionInfos.push({
				...
				expression: '($feature.REGISTERED_VOTERS-(' + AllFeatures + '))',
			});
			...
			arcadeExpressionInfos.push({
				name: 'CastPercent',
				title: 'Cast Percent',
				expression: 'Round(' + '((' + AllFeatures + ')/$feature.REGISTERED_VOTERS)*100, 1)',
			});

			//Arcade Expression: Not Cast Percent
			arcadeExpressionInfos.push({
				name: 'NotCastPercent',
				title: 'Not Cast Percent',
				expression:
					'Round((($feature.REGISTERED_VOTERS-(' + AllFeatures + '))/$feature.REGISTERED_VOTERS)*100, 1)',
			});
			...
			arcadeExpressionInfos.push({
				...
				title: 'Not Cast Bar',
				expression:
					'When( ($feature.REGISTERED_VOTERS-(' +
					AllFeatures +
					')) < (' +
					AllFeatures +
					'), (($feature.REGISTERED_VOTERS-(' +
	...
	function AssignColor(CandidateFieldName) {
		var Color;
		if (
			CandidateColorList.findIndex(function(element) {
				return element == CandidateFieldName;
			}) == -1
		) {
			...
			for (var l = 0; l < PartyColorList.length; l++) {
				if (
					ColorList.findIndex(function(element) {
						return element == PartyColorList[l];
					}) == -1
				) {
	...
	function CreateTurnoutTable() {
		...
		var FooterText = document.createTextNode(
			DivRaceTotal + ' votes were cast, with ' + DivTotalRegisteredVoters + ' registered voters for this race.'
		);
	...
	function CreateMapOptionRadios() {
		...
		ToolTip.appendChild(
			document.createTextNode('Percent of the candidate recieving the most votes in the precinct')
		);
	...
	function changeRadioSymbology(fieldName) {
		if (fieldName === 'Label') {
			...
			var opacityVisVar = {
				...
				stops: [{ value: PercentMax, opacity: 1 }, { value: PercentMin, opacity: 0.25 }],
			};
			...
			var opacityVisVar = {
				...
				valueExpression:
					...
					NewListStatement +
					')[' +
					String(GloablFieldList.length - 1) +
					'] - Sort(' +
					NewListStatement +
					')[' +
					String(GloablFieldList.length - 2) +
					'] )',
				stops: [{ value: MarginMax, opacity: 1 }, { value: MarginMin, opacity: 0.25 }],
			};
			var renderer = OnLayer.renderer.clone();
			renderer.visualVariables = [opacityVisVar];
			OnLayer.renderer = renderer;
			...
			var opacityVisVar = {
				...
				stops: [{ value: TurnoutMax, opacity: 1 }, { value: TurnoutMin, opacity: 0.25 }],
			};
			var renderer = OnLayer.renderer.clone();
			renderer.visualVariables = [opacityVisVar];
			OnLayer.renderer = renderer;
